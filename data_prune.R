...
subset_mutcode <- grepl("(^|,|\\s+)(S?)SF|(^|,|\\s+)(S?)NSF|(^|,|\\s+)E($|,)|TCR|DR", ctl_variant$Mutation.Type.Code) # the mutation type code has to be one of the following: E, NSF, NSF-2, SF, SNSF, SSF, TCR, DR
...
function_code_to_immunogenicity <- function(row_of_ctl){ # this function is used to transfer the mutatant code to a immunogenicity score which measures the strength of the mutation epitope - CTL response
  ...
  total_code <- sum(grepl("(^|,|\\s+)(S?)SF|(^|,|\\s+)(S?)NSF|(^|,|\\s+)E($|,)|TCR|DR", codes[[1]])) # the total number of codes related to the change of immunogenicity
...
write.csv(ctl_variant_pruned, file = "ctl_variant_pruned.csv")
