# rewrite the formular of Ec in the matrix format, implement the recursive training with x and p fixed alternatively
...
A <- peptide_to_matrix(c("A", "A", "A", "A"), 3, aa_name, 3, 4)
p <- rep(1, 22)
x <- rep(2, 40)

#cost <- costfunction_fixx(p, x, ctl_binded_combinded[1:2, ], 0, len_pep, aa_name, allele_name)
#pinter <- d_costfunction_fixx(p, x, ctl_binded_combinded[1:2, ], 0, len_pep, aa_name, allele_name )
...
#res <- optim(runif(40, -0.8, 0.8), costfunction_fixp, d_costfunction_fixp, c(1,p_wif), ctl_binded_combinded, 0, 9, aa_name, allele_name, method = "BFGS") # this currently gives the best result
#x<- res$par
#res <- optim(c(1,p_wif), costfunction_fixx, d_costfunction_fixx, x, ctl_binded_combinded, 0, 9, aa_name, allele_name, method = "BFGS")
#p <- res$par
#res <- optim(x, costfunction_fixp, d_costfunction_fixp, p, ctl_binded_combinded, 0, 9, aa_name, allele_name, method = "BFGS") # this currently gives the best result
#x<- res$par

x <- rep(1, length(allele_name) * (len_pep + 1)) # this is the initial x
p <- c(1, p_Sdandinski) # this is the initial p
#p <- c(1, rep(-1,5), rep(0, 9), rep(1, 7))
for (i in 1:10){
  res <- optim(x, costfunction_fixp, d_costfunction_fixp, p, ctl_binded_combinded[1:140, ], 0, len_pep, aa_name, allele_name, method = "BFGS") # this currently gives the best result
  x<- res$par
  print(res$value)
  res <- optim(p, costfunction_fixx, d_costfunction_fixx, x, ctl_binded_combinded[1:140, ], 0, len_pep, aa_name, allele_name, method = "BFGS")
  p <- res$par
  print(res$value)
}

#x <- rep(1, 40) # this is the initial x
#p <- c(1, p_wif) # this is the initial p
#for (i in 1:10){
#  res <- optim(p, costfunction_fixx, d_costfunction_fixx, x, ctl_binded_combinded, 0, len_pep, aa_name, allele_name, method = "Nelder-Mead")
#  p <- res$par
#  print(res$value)
#  res <- optim(x, costfunction_fixp, d_costfunction_fixp, p, ctl_binded_combinded, 0, len_pep, aa_name, allele_name, method = "Nelder-Mead") # this currently gives the best result
#  x<- res$par
#  print(res$value)
#}

pred <- evaluate(p, x, ctl_binded_combinded[1:140,], 0, len_pep, aa_name, allele_name) # see what prediction is made by the trained model
exper_pred <- cbind(ctl_binded_combinded[1:140,]$Immunogenicity, round(pred)) # this is easy for us to see which prediction is right, which prediction is wrong
...
# currently it is good to just look at the 9 mers, since it yeilds the best results
plot(factor(aa_name), -p[2:length(p)])

# count the occurences of each characters 
freq <- function(word, letter){
  sum(charToRaw(word) == charToRaw(letter))
}

freq_in_database <- function(letter, peps){
  sum(sapply(peps, freq, letter))/(length(peps) * len_pep)
}

freq_aa <- sapply(aa_name, freq_in_database, ctl_A0201_binded$peptide) # the occurence frequency of W is very low, that is why we always get super large W
barplot(freq_aa)

# count the occurences of each aa at each position
freq_position <- sapply(1:9, function(i){
  sapply(aa_name, freq_in_database, substr(ctl_A0201_binded$peptide, i, i))
})
barplot(freq_position[,1]) # aa at position 2 and 9 are fairly conserved
...
# without regularization, we have 20% error rate for the training set and 30% for the test set
...
for (i in 1:10){
  res <- optim(x, costfunction_fixp, d_costfunction_fixp, p, train_set, 0.0, len_pep, aa_name, allele_name, method = "BFGS") # this currently gives the best result
  x<- res$par
  print(res$value)
  res <- optim(p, costfunction_fixx, d_costfunction_fixx, x, train_set, 0.5, len_pep, aa_name, allele_name, method = "BFGS")
  p <- res$par
  print(res$value)
}
