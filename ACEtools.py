...
def getstates(count, qcut, byEntropy=False, byFrequency=False, gauge='cons', useX=False, use21=False):
    ...
    if gauge not in ['cons', 'least', 'group']: gaugeseq = np.loadtxt(gauge, dtype = np.str)
    ...
    # Compress the number of states
        ...
        for c in count:
            ...
            else:                                       gaugeidx = AAsort.tolist().index(gaugeseq[i])
...
def consensus2zerosum(h, J):
    ...
    newJ = []
    h_new = np.hstack((h, np.zeros((N, 1))))
    ...
    for i, j in pairs(N):
        ...
        for a in range(q[i]):
            ...
            # change the gauge for h
            h_new[i][a] += (suma[a] * invq[j]) - (sumall * invq[i] * invq[j])/2

        # change the gauge for the gauge state
        h_new[i][q[i]] += -(sumall * invq[i] * invq[j])/2
        ...
        for b in range(q[j]):
            ...
            # change the guage for h
            h_new[j][b] += (sumb[b] * invq[i]) - (sumall * invq[i] * invq[j])/2

        # change the guage for the guage state
        h_new[j][q[j]] += - (sumall * invq[i] * invq[j])/2
    ...
    # Return new vector h in zero sum gauge

    return newJ, (h_new.transpose() - h_new.mean(axis=1)).transpose()
