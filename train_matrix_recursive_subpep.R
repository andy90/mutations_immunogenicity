...
#ctl_A0201_binded_9mer <- ctl_A0201_binded[nchar(ctl_A0201_binded$peptide) == 9, ] # only look at the 9 mers
#ctl_A0201_binded_9mer$Immunogenicity <- round(ctl_A0201_binded_9mer$Immunogenicity) # round the immunogenicity to 0 or 1
#ctl_A0201_binded_9mer$peptide <- substr(ctl_A0201_binded_9mer$peptide, 3, 8) # only look at the middle part of the peptide
...
ctl_binded_9mer$peptide <- substr(ctl_binded_9mer$peptide, 1, 9)
ctl_binded_9mer_reshuffle <- ctl_binded_9mer[sample(nrow(ctl_binded_9mer)), ]

index_train_set <- round(nrow(ctl_binded_9mer_reshuffle)*0.7)
train_set <- ctl_binded_9mer_reshuffle[1:index_train_set, ]
test_set <- ctl_binded_9mer_reshuffle[(index_train_set+1):nrow(ctl_binded_9mer_reshuffle), ]
...
# when only include 4-6, there is about 20% miss prediction for train, and 30% for test
# if we don't round the immunogenicity to 0 or 1 before training, the result seems to be worse
# when only include 3-5, there is about 20% miss prediction for train, and 30% for test # the fluctuation is really big when divide by 70 and 30
# when only look at 5, there is about 30% miss prediction for train, and 30% for test, the hydrophobicity score obtained fluctuates a lot for each run
# when only look at 3, the results is similar to when only look at 4, the hydrophobicity score obtained fluctuates less
# when only look at 4, the average error rate is similar, although the fluctuation of the error rate is bigger than 3 or 5
# similar or a little bit worse for 6
# 7 is similar to 6
# when onlly include 3-8, there is about 20% error rate for train and 30% for test, these are all just for A0201 alleles

# when only include 3-8 for all the alleles there is about 20% error rate for train and more than 30% for test. The fluctuation of hydrophobicity score is still there, but much less than when only using 1 alleles
# when only inlcude 4-6 for all the alleles there is more than 20% error rate for train and more than 30% for test. The fluctuation of hydrophobicity is still pretty big